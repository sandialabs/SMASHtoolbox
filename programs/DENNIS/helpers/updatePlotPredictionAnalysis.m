...
% plot x-ray source
    ...
    function plotXray
        ...
        set(findobj(db, 'Tag', 'PolFrac'), 'Value', ...
            num2str(num2str(obj.source.polarizationFraction)));
        set(findobj(db, 'Tag', 'PolX'), 'Value', ...
            num2str(num2str(obj.source.polarizationVector(1))));
        set(findobj(db, 'Tag', 'PolY'), 'Value', ...
            num2str(num2str(obj.source.polarizationVector(2))));
        set(findobj(db, 'Tag', 'PolZ'), 'Value', ...
            num2str(num2str(obj.source.polarizationVector(3))));
...
% plot detector
    ...
    function plotDetector
        ...
        elseif ~isnumeric(obj.simulation.image) && ...
            ...
            switch obj.simulation.image.GraphicOptions.YDir
                case 'normal'
                    so = [3 4 2 1];
                case 'reverse'
                    so = [2 1 3 4];
            ...
            if obj.simulation.displayLabelsInDENNIS && ...
                ...
                xvec = b/norm(b);
                yvec = a/norm(a);
                if strcmp(obj.simulation.image.GraphicOptions.YDir, ...
                        'reverse')
                    yvec = -yvec;
                end
            ...
            switch obj.detector.image.GraphicOptions.YDir
                case 'normal'
                    so = [3 4 2 1];
                case 'reverse'
                    so = [2 1 3 4];
