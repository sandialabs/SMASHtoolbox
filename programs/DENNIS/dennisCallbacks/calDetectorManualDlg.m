...
h_poiButton = findobj(new, 'Tag', 'Auto');
h_poiButtonMan = findobj(new, 'Tag', 'Manual');
...
if contains(obj.calibration.type, 'detector')
    previousPOI = obj.calibration.poi;
end
obj = changeObject(obj, 'calibration', 'type', 'detectorManual');
obj = calPOI(obj, previousPOI);
...
set(h_poiButtonMan, 'ButtonPushedFcn', {@pressManualButton, mainFigure, ax});
...
obj = get(mainFigure, 'UserData');
obj = calProcessImage(obj, 'mask', ax, {'connected', 'guess', ...
    0.001*(obj.detector.image.Grid1(end)-obj.detector.image.Grid1(1))});
view(obj.calibration.processedImage, 'show', ax);
delete(findobj(ancestor(ax,'figure'), 'type', 'colorbar'));
axis(ax, 'equal');
set(mainFigure, 'UserData', obj);
...
obj = get(mainFigure, 'UserData');
obj = calCC(obj);
obj = changeObject(obj, 'calibration', 'opts', 'poiType', 'auto');
obj = calPOI(obj);
plotPOI(obj,ax);
set(mainFigure, 'UserData', obj);
end

function pressManualButton(source, event, mainFigure, ax)
delete(findobj(ax,'Type','Line'));
obj = get(mainFigure, 'UserData');
obj = changeObject(obj, 'calibration', 'opts', 'poiType', 'manual');
obj = calPOI(obj, ax);
plotPOI(obj,ax);
set(mainFigure, 'UserData', obj);
end
...
function plotPOI(obj,ax)
delete(findobj(ax,'Type','Line'));
if isempty(obj.calibration.poi) % for some reason a [0x0] has a size(x,3) of 1
    return
end
