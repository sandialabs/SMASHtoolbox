...
h_fillEdit = findobj(new, 'Tag', 'FillEdit');
h_fillButton = findobj(new, 'Tag', 'FillButton');
h_fillReset = findobj(new, 'Tag', 'FillReset');
...
set(ax, 'FontSize', 16);
hold(ax, 'on')
if str2double(mainFigure.UserData.externalUserData.version(1:end-1)) > 2025
...
set(h_fillEdit(1), 'Value', num2str(1));
set(h_fillEdit(2), 'Value', num2str(3));
...
set(h_fillButton, 'ButtonPushedFcn', {@processFill, mainFigure, h_fillEdit});
set(h_fillReset, 'ButtonPushedFcn', {@processFillReset, mainFigure});
...
ax.Toolbar.SelectionChangedFcn = @reShow;
...
function processFill(src, event, mainFigure, h_fillEdit)
fillVal = editExtract(h_fillEdit);
if all(~isnan(fillVal))
    obj = get(mainFigure, 'UserData');
    obj = processImage(obj, 'fillmissing', fillVal(1), 'movmedian', ...
        fillVal(2));
    set(mainFigure, 'UserData', obj);
    updatePlotPredictionAnalysis(mainFigure);
else
    errordlg('Bad fill missing inputs', 'Bad fill missing inputs')
end
end

function processFillReset(src, event, mainFigure)
obj = get(mainFigure, 'UserData');
obj = processImage(obj, 'fillmissingreset');
set(mainFigure, 'UserData', obj);
updatePlotPredictionAnalysis(mainFigure, 'imageProcessingDisplay');
end
