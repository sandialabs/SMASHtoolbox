...
switch lower(mode)
    ...
    case 'full'
        ...
        data(1,:)=record.Time;
        data(2,:)=record.Velocity;
        if all(size(record.Phase) == size(record.Time))
            data(3,:)=record.Phase;
            warning('No phase info - written as -1')
        else
            data(3,:) = -1;
        end
        if isfield(record, 'BIM') && all(size(record.BIM) == size(record.Time))
            data(4,:)=record.BIM;
            warning('No BIM info - written as -1')
        else
            data(4,:) = -1;
        end
