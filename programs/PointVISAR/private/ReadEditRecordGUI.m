...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function ShowBounds(record,h)

% remove old bounding lines
hb=findobj(h,'Tag','BoundLine');
delete(hb);
...
for ii=1:2
    if ~isinf(initial(ii))
        hl(end+1)=line(h,'XData',repmat(initial(ii),[1 2]),'YData',ylim(h),'LineStyle','-.');
    end
    %if ~isinf(ellipse(ii))
    %    hl(end+1)=line('XData',repmat(ellipse(ii),[1 2]),'YData',ylim,'LineStyle',':');
    %end
    if ~isinf(experiment(ii))
        hl(end+1)=line(h,'XData',repmat(experiment(ii),[1 2]),'YData',ylim(h),'LineStyle','--');
    end
end
...
switch choice        
    ...
    case 'InitialTime'
        ...
        DoubleInputDlg_dynamic(prompt, 'Initial velocity duration', ...
            fig, 'InitialTime');
        return
    %case 'EllipseTime'
    %    prompt = {'Begin time', 'End time'};
    ...
    case 'ExperimentTime'
        prompt = {'Begin time', 'End time'};
        DoubleInputDlg_dynamic(prompt, 'Experiment duration', ...
            fig, 'ExperimentTime');
        return
end
...
switch choice
    ...
    case 'SetTimeShifts'
        DoubleInputDlg_dynamic(record.SignalLabels, 'Time shifts', ...
            fig, 'SignalTimeShift');
        return
    case 'SetTimeScale'
    ...
    case 'SetVerticalShifts'
        DoubleInputDlg_dynamic(record.SignalLabels, 'Vertical shifts', ...
            fig, 'SignalVerticalShift');
        return  
    case 'SetSignalScaling'
        DoubleInputDlg_dynamic(record.SignalLabels, 'Signal scaling', ...
            fig, 'SignalScale');
        return           
end
...
delete(findall(0,'Type', 'figure', 'Tag', 'DoubleInputDlg_dynamic'));
