...
%end
filterspec={...
    '*.*','All files'}; % otherwise, MATLAB now will put the All files option at the bottom - super annoying behavior
...
figure(ancestor(src, 'figure')); % had to add for javascript-based matlab
...
if strcmp(choice,'ok') % user chose OK button
    ...
    if numel(InputFiles) == 1
        [~, record.Label] = fileparts(InputFiles{1});
    end
