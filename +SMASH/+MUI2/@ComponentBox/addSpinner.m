% addSpinner Add edit block
% 
% This method adds a spinner block to the current row.
%    h=addSpinner(object,width);
% Optional input "width" defines the number of horizontal characters that
% must fit inside the spinner box, using default value of 10.  Output "h" a
% two-component handle array for the uilabel and uispinner components.
...
% spinner components.  The second approach is particularly useful when the
% label is placed to the left of the spinner.  Distinct component sizing is
% supported but usually unneccessary for labels above the spinner.
%
% See also ComponentBox, setLabelPosition, uilabel, uispinner
%
function h=addSpinner(object,width)
