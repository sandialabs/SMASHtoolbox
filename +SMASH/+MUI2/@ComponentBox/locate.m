% Locate Move control box on screen
%
% This method adjusts the location of a component box.  The position of
...
% supported.
%
% See also ComponentBox, fit, hide, show
%
...
elseif reference==0 % root object    
    movegui(object.Figure,location);
else % another figure
    units=get(reference,'Units');
    set(reference,'Units','pixels');
    ref=get(reference,'Position');    
    if strcmpi(reference.WindowStyle,'docked')        
        message{1}='Docked figure position cannot be accurately referenced';
        message{end+1}='Component box centered on display';
        warning('ComponentBox:DockedFigures','%s\n',message{:});
        locate(object,'center');
        return
    end
    pos=get(object.Figure,'Position');
    ...
    set(object.Figure,'Position',pos);
    movegui(object.Figure,'onscreen'); % make sure dialog appears on the screen
