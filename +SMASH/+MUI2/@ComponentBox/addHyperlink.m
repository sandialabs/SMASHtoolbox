% addHyperlink Add hyperlink block
%
% This method adds a hyperlink block to the current row.
%    h=addHyperlink(object,width,number);
% Optional input "width" defines the number of horizontal characters that
% must fit inside each hyperlink; the default value is 10.  Optional input
% "number" indicates the number of vertically-stacked hyperlinks to be
% created with the current gap setting.  Output "h" is a handle array of
% uihyperlink components.
%
% See also ComponentBox, uihyperlink
%
function h=addHyperlink(object,width,number)
