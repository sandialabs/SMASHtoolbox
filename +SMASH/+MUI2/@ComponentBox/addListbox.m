% addListbox Add list box block
%
% This method adds a list box block to the current row.
%    h=addListbox(object,width,rows);
% Optional input "width" defines the number of horizontal characters that
% must fit inside the list box, using default value of 10.  Optional input
% "rows" defines the vertical size of the list box, with a default value of
% 4.  Output "h" a two-component handle array for the uilabel and uilistbox
...
% list box components.  The second approach is particularly useful when the
% label is placed to the left of the list box.  Distinct component sizing
% is supported but usually unneccessary for labels above the list box.
%
% See also ComponentBox, setLabelPosition, uilabel, uidropdown
%
function h=addListbox(object,width,rows)
