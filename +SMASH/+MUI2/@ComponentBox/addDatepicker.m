% addDatepicker Add date picker block
% 
% This method adds an date picker block to the current row.
%    h=addDatepicker(object,width,style);
% Optional input "width" defines the number of horizontal characters that
% must fit inside the date picker box, using default value of 10.  Output
% "h" a two-component handle array for the uilabel and uidatepicker
...
% date picker components.  The second approach is particularly useful when
% the label is placed to the left of the date picker box.  Distinct
% component sizing is supported but usually unneccessary for labels above
% the date picker.
% 
% See also ComponentBox, setLabelPosition, uidatepicker, uilabel
%
function h=addDatepicker(object,width)
...
h(2)=uidatepicker(object.Figure);
for n=1:2
