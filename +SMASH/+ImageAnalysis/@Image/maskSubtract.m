% MASKSUBTRACT Mask and subtract data from an image
%
% Interactive selection of mask and subsequent subtraction of masked data
% from image
%     >> object=maskSubtract(object) % interactive selection
%     >> object=maskSubtract(object,target) % interactively select region on a
...
%     >> object=maskSubtract(object,target, invertFlag) % specify inversion 
%                                               (default is false)
%     >> object=maskSubtract(object,ind) % manually input mask index
%     >> object=maskSubtract(object,ind, invertFlag) % specify inversion 
%                                               (default is false)
%     
% 
%
...
if any(isnan(ind))
    ...
    if strcmpi(roiInfo{1}, 'connected')
        if strcmpi(roiInfo{2}, 'guess')
            xdiff = abs(xv(end) - xv(1));
            ydiff = abs(yv(end) - yv(1));
            if xdiff > ydiff
                roiInfo{2} = 'x';
            else
                roiInfo{2} = 'y';
        ...
        switch lower(roiInfo{2})
            case 'y'
                w = max([diff(Grid1(1:2)), roiInfo{3}]);
                xv = [xv-w; xv(end:-1:1)+w];
                yv = [yv; yv(end:-1:1)];
            case 'x'
                w = max([diff(Grid2(1:2)), roiInfo{3}]);
                yv = [yv-w; yv(end:-1:1)+w];
                xv = [xv; xv(end:-1:1)];
        end
    end
