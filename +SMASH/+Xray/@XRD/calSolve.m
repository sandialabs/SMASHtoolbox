...
if numel(twoThetaPred) < size(poi,3)
    errordlg(['Mismatch between number of cal regions and number of ', ...
        'predicted rings on the detector. Check detector position ', ...
        'and consider using manual ring POI selection instead of auto.']);
end
...
assert(~isempty(hkl), ...
    'Failed to find any candidate hkl: check spectrum width');
