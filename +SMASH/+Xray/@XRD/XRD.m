...
classdef XRD
    ...
    methods
        ...
        function obj = XRD
            ...
            obj = changeObject(obj, 'source', 'location', [0 -10 0]);
            ...
            obj.source.polarizationFraction = 0; % unpolarized
            obj.source.polarizationVector = [1 0 0]; % horizontal
        ...
        obj = calExcludePOI(obj, varargin);
        [obj, score, gaInfo] = calSolve(obj);
        obj = calProcessImage(obj, varargin);
        varargout = matchImage(obj, varargin)
        [obj, varargout] = matchProcessImage(obj, processType, varargin)
        varargout = matchCentroids(obj, varargin)
        varargout = setMatchDefaults(obj,varargin);
        varargout = applySimGridToDet(obj,varargin);
        varargout = resetMatch(obj,varargin);
        varargout = matchROISplit(obj,varargin);
        varargout = matchROIProcess(obj,varargin);
        varargout = matchWeightMultiplier(obj,varargin);
        varargout = matchROI(obj,varargin);
        varargout = matchPOIExclude(obj,varargin);
        varargout = matchPOI(obj,varargin);
        varargout = matchCC(obj,varargin);
        varargout = matchPOIView(obj,varargin);
        varargout = generateCake(obj,varargin);
        varargout = matchRings(obj,varargin);
    ...
    methods (Access = private)
        ...
        obj = generatehkl(obj, varargin);
        obj = deleteDetectorHistory(obj, type, everything)
        obj = applyPolarizationCorrection(obj, varargin);
    end
    ...
    methods (Access = private, Static = true)
        ...
        varargout = computeSimulationImageDimensions(varargin)
        varargout = applySimGridToImage(varargin);
        varargout = autoSpotFind(varargin);
        varargout = findCC(varargin);
        varargout = findCrystalPOI(varargin);
        varargout = showImage(varargin);
        varargout = checkAngleBounds(varargin);
    end

    methods % Easter egg
        function [obj, h, im] = bakeCake(varargin)
            [obj, h, im] = generateCake(varargin{:});
        end
    end
