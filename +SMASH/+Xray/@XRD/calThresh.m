...
if strcmp(obj.calibration.type, 'detectorAuto')
    ...
    im = replace(im, fillmissing(im.Data,'nearest'), ...
        true(size(im.Data)));
    im = bandpass(im, obj.calibration.opts.filterBounds, 'butterworth');
...
switch obj.calibration.type
    case 'detectorAuto'
        ...
        if numel(unique(peakInd)) ~= numel(peakInd)
            errordlg(['Failed to find unique peaks across the ', ...
                'user selection. Use manual detector calibration.']);
        end
