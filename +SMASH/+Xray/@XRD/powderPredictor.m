...
%
% This method predicts the uncorrected-intensity powder diffraction pattern
%
% Usage:
...
%    in their source, doi: 10.1107/S0108767394013292). The final corrected 
%    intensity values will differ from VESTA because VESTA assumes
%    the standard slit correction that is not applicable to area detectors
%
...
% compute uncorrected intensities

if ~simFlag || ~obj.simulation.uniformSpotIntensity
    obj.(fieldName).I = F_abs.^2;
end
...
if simFlag
    ...
    obj.(fieldName) = rmfield(obj.(fieldName),'lambdaSol');
    ...
    obj.(fieldName).xy(badInd,:) = [];
    obj.(fieldName).s(badInd,:) = [];
    obj.(fieldName).twoTheta = 2*asind(sinTheta(~badInd));
