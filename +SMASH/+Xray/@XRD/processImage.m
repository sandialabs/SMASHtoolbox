...
%
% This method applies image processing to the loaded detector image.
%
% Usage:
...
switch lower(processType)
    ...
    case 'smooth'
        ...
        imSmooth = smooth(im, varargin{:});

        % apply pre-existing masking and backgrounding to maintain prior
        % zero values

        obj.detector.image = replace(imSmooth, 0, im.Data == 0);
    ...
    case 'fillmissing'
        ...
        if isnumeric(obj.detector.imageHistory.prefillmissing)
            obj.detector.imageHistory.prefillmissing = im;
        end

        assert(nargin > 2, 'Missing required threshold input');
        imFill = replace(im, nan, im.Data <= varargin{1});
        varargin(1) = [];
        imFill = fillmissing(imFill, varargin{:});
        obj.detector.image = replace(imFill, 0, im.Data == 0); % maintain mask

    case 'fillmissingreset'

        if ~isnumeric(obj.detector.imageHistory.prefillmissing)
            obj.detector.image = obj.detector.imageHistory.prefillmissing;
    ...
    case 'reverttooriginal'
        ...
        im = obj.detector.imageHistory.original;
        obj.detector.image = im;
        obj.detector.imageHistory.original = im;
        
    case 'reverttosave'
        
        im = obj.detector.imageHistory.lastsave;
        imOr = obj.detector.imageHistory.original;
        obj.detector.image = im;
        obj.detector.imageHistory.lastsave = im;
        obj.detector.imageHistory.original = imOr;
