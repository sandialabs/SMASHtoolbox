...
for n=1:2:Narg
    ...
    switch lower(name)
        ...
        case 'increment'
            assert(islogical(value), 'ERROR: Increment must be logical')
...
setName(cb, option.FigureName);
setLabelPosition(cb,'above');

rows=10;
ROIlist=addListbox(cb,25,rows);
set(ROIlist(1),'Text',option.Title);
updateList();
showSelection();
...
waitfor(cb.Figure);
    ...
    function modifyAction(varargin)
        ...
        updateList();
        view(object(index),option.Target);
        show(cb); show(cb); % shouldn't need twice... matlab graphics issue
    end
    function addAction(varargin)
        object=add(object,mode,name);
        updateList();
        wipe(object,option.Target);
