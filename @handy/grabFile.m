...
%
% This *static* method makes a local copy of specified file.  Unlike the
% builtin copyfile feature, the absolute location of the file is not
% required. Users can access information adjacent to a feature of interest
% without that file being on the MATLAB path.  For example, a script
% 'exampleA.m' might be stashed inside a folder 'examples' next to a
% function/class file that demonstrates the use of that file.  Users can
% grab a local copy of that script to better understand the function/class
% without concern that the original version has been modified; this is
% particularly important when the function/class is stored in a version
% control repository.
%
% Generating a local copy depends on two mandatory inputs.
%    handy.grabFile(source,name);
...
%    handy.grabFile('myPack.myFunc','examples/exampleA.m');
...
% wildcard (such as *.m), the matching contents are printed to the
% command window:
%     handy.grabFile(source,'examples');
% or returned as a cell array of character vectors.
%     list=handy.grabFile(source,'examples/*.m');
...
%
% Files copied by this function are stored in a subfolder named
% 'grabFile_folder'.  The absolute location of the copied file is returned
%c as an output:
%    new=handy.grabFile(source,name);
...
%    handy.grabFile(source,name,'show');   % default behavior
%    handy.grabFile(source,name,'noshow'); % copied file not opened
%
% See also handy, copyfile, doc
%
% NOTE: although this function will copy any requested file, it is meant
...
if strcmpi(source,'-link') % for use with doc hyperlink
    ...
    source=handy.superWhich(source);
