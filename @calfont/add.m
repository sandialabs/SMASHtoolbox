...
%    calfont.add(name,points);
% Optional inputs "name" and "points" indicate the desired font and its
% name.  Empty arguments invoke defaults based on the standard uilabel
% font.
%    calfont.add('',points); % default font, specified size
...
% default choice.
%
% Font sizes can be also be specified in terms of vertical rows.
%    calfont.add(name,value,'rows');
% The number of points derived from "value" allows *roughly* that many text
...
% font, except when no other fonts have been calibrated.  The choose
...
%
% See also calfont, check, choose, lookup, query, remove, show
%
...
start=tic();
fprintf('Calibrating %s %d point font...',name,points);
...
%set(hl,'FontName',name,'FontSize',points);
set(hl,'FontName',name,'FontSize',points*calfont.scale());
...
new=struct('Name',name,'Size',points);
...
match=false();
for n=1:numel(cal)
    if strcmpi(cal(n).Name,name) && (cal(n).Size == points)
        match=true();
        cal(n)=new;
        break
    end
